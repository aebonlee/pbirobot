...
const OrderHistory = (): ReactElement => {
  ...
  const formatPrice = (price: number | undefined): string => {
    return isEn
      ...
      : `${Number(price || 0).toLocaleString()}${t('shop.currency') || '원'}`;
  };
          ...
          <div style={{ maxWidth: '800px', margin: '0 auto' }}>
            {loading ? (
              ...
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                ...
                <button onClick={loadOrders} className="btn btn-primary" style={{ marginRight: '12px' }}>
                  {t('auth.retry')}
                </button>
                <Link to="/mypage" className="btn btn-ghost">{t('auth.myPage')}</Link>
              </div>
            ) : orders.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '60px 0' }}>
                <div style={{
                ...
                <Link to="/store" className="btn btn-primary">{t('order.backToShop')}</Link>
              </div>
            ) : (
            ...
            <div style={{ textAlign: 'center', marginTop: '32px' }}>
              <Link to="/mypage" className="btn btn-ghost" style={{ marginRight: '12px' }}>{t('auth.myPage')}</Link>
              <Link to="/store" className="btn btn-ghost">{t('order.backToShop')}</Link>
            </div>
          </div>
        </div>
      </section>
