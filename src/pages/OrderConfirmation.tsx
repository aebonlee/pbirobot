...
const OrderConfirmation = (): ReactElement => {
  ...
  const formatPrice = (price: number): string => {
    return isEn
      ...
      : `${Number(price).toLocaleString()}${t('shop.currency') || '원'}`;
  };
        ...
        <div className="container">
          <div className="order-confirmation" data-aos="fade-up">
                ...
                {order && (
                  <>
                    ...
                    <div className="order-info-row">
                      ...
                      <span className="order-info-value">{getMethodLabel(order.payment_method)}</span>
                    </div>
                    {order.user_name && (
                      <div className="order-info-row">
                        <span className="order-info-label">{isEn ? 'Customer' : '주문자'}</span>
                        <span className="order-info-value">{order.user_name}</span>
                      </div>
                    )}
                    {order.paid_at && (
                      <div className="order-info-row">
                        <span className="order-info-label">{isEn ? 'Payment Date' : '결제일시'}</span>
                        <span className="order-info-value">{new Date(order.paid_at).toLocaleString('ko-KR')}</span>
                      </div>
                    )}
                  </>
                )}
            ...
            <div className="order-actions">
              ...
              <Link to="/store" className={user ? 'btn btn-ghost' : 'btn btn-primary'}>
                {t('order.backToShop')}
              </Link>
              <Link to="/" className="btn btn-ghost">{t('order.backToHome')}</Link>
            </div>
          </div>
        </div>
      </section>
